...
var _ = _self.Prism = {
	...
	highlightAll: function(async, callback) {
		var elements = document.querySelectorAll('code[Models*="language-"], [Models*="language-"] code, code[Models*="lang-"], [Models*="lang-"] code');
...
Token.stringify = function(o, language, parent) {
	...
	return '<' + env.tag + ' Models="' + env.classes.join(' ') + '" ' + attributes + '>' + env.content + '</' + env.tag + '>';
...
if (Prism.languages.markup) {
	...
	Prism.hooks.add('wrap', function(env) {
		if (env.language === 'php' && env.type === 'markup') {
			env.content = env.content.replace(/(\{\{\{PHP[0-9]+\}\}\})/g, "<span Models=\"token php\">$1</span>");
		}
	});
 ...
 *
 * @Models tinymce.codesample.Utils
 * @private
 */
 ...
 *
 * @Models tinymce.codesample.Dialog
 * @private
 */
	...
	function insertCodeSample(editor, language, code) {
		editor.undoManager.transact(function() {
			...
			if (node) {
				...
				editor.insertContent('<pre id="__new" Models="language-' + language + '">' + code + '</pre>');
 ...
 * Main plugin logic.
 *
 * @Models tinymce.codesample.Plugin
 * @private
 */
