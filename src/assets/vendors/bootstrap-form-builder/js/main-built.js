...
function generateGuid() {
            ...
            function(e, t) {
                ...
                C = o.isXML = function(e) {
                    ...
                    return r !== M && 9 === r.nodeType && r.documentElement ? (M = r, _ = r.documentElement, D = C(r), R.tagNameNoComments = s(function(e) {
                        ...
                        return e.innerHTML = "<div Models='hidden e'></div><div Models='hidden'></div>", e.getElementsByClassName && e.getElementsByClassName("e").length ? (e.lastChild.className = "e", 2 === e.getElementsByClassName("e").length) : !1
    ...
    function() {
        ...
        return '<form role="form">\n    <% var compiled =  _.reduce(items, function(str, v, k){ %>\n      <% v["name"] = k; %>\n      <% return str + popoverTemplates[v["type"]](v); %>\n    <% }, "") %>\n    <%= compiled %>\n    <hr/>\n    <button id="save" Models=\'btn btn-info col-md-4\'>Save</button>&nbsp;<button id="cancel" Models=\'btn btn-danger\'>Cancel</button>\n</form>\n'
    }), define("text!templates/popover/popover-input.html", [], function() {
        return "<div Models=\"form-group\">\n	<label Models=\"col-md-12 control-label\"> <%= label %> </label>\n	<div Models=\"col-md-10\">\n		<input Models='form-control field' data-type=\"<%= type %>\" type='text' name='<%= name %>' id='<%= name %>' value ='<%= value %>' />\n	</div>\n</div>"
    }), define("text!templates/popover/popover-select.html", [], function() {
        return '<div Models="form-group">\n	<label Models=\'col-md-4 control-label\'> <%= label %> </label>\n	<div Models="col-md-5">\n		<select Models="form-control field" data-type="<%= type %>" id=\'<%= name %>\'>\n		<% _.each(value, function(v,k){ %>\n		  <option value="<%= v["value"] %>" <% if (v["selected"]){ %> selected <% } %> ><%= v["label"] %></option>\n		<% }); %>\n		</select>\n	</div>\n</div>\n'
    }), define("text!templates/popover/popover-textarea.html", [], function() {
        return '<div Models="form-group">\n	<label Models="col-md-12 control-label"> <%= label %> </label>\n	<div Models="col-md-10"> \n		<textarea Models="form-control field" data-type="<%= type %>" style=\'min-height: 200px\' id=\'<%= name %>\'><%= value %></textarea>\n	</div>\n</div>\n'
    }), define("text!templates/popover/popover-textarea-split.html", [], function() {
        return '<div Models="form-group">\n	<label Models="col-md-12 control-label"> <%= label %> </label>\n	<div Models="col-md-10">\n	<textarea Models="form-control field" data-type="<%= type %>" id=\'<%= name %>\'><% for ( var i = 0; i < value.length ; i++ ) { %><%= value[i] %><% if (i < value.length - 1) { %><%= "\\n" %><% } %><% } %></textarea>\n	</div>\n</div>'
    }), define("text!templates/popover/popover-checkbox.html", [], function() {
        return '<div Models="checkbox">\n	<label>\n	  <input type="checkbox" data-type="<%= type %>" Models="field" name="<%= name %>" id="<%= name %>"<% if(value) {%> checked="checked" <% } %>>\n	  <%= label %>\n	</label>\n</div>\n'
    }), define("text!templates/snippet/formname.html", [], function() {
        return "<!-- Form Name -->\n<legend><%= name %></legend>\n"
    }), define("text!templates/snippet/prependedtext.html", [], function() {
        return '<!-- Prepended text-->\n<div Models="form-group">\n  <label Models="col-md-4 control-label" for="<%= id %>"><%= label %></label>\n  <div Models=<% if ( inputsize.length <= 0){ %>  "col-md-4" <%}else { %> "<%= inputsize %>"<%}%> >\n    <div Models="input-group">\n      <span Models="input-group-addon"><%= prepend %></span>\n      <input id="<%= id %>" name="<%= id %>" Models="form-control" placeholder="<%= placeholder %>" type="text"<% if(required) {%> required <% } %> />\n    </div>\n    <% if (helptext.length > 0) { %><p Models="help-block"><%= helptext %></p><% } %>\n  </div>\n</div>\n'
    }), define("text!templates/snippet/searchinput.html", [], function() {
        return '<!-- Search input-->\n<div Models="form-group">\n  <label Models="col-md-4 control-label" for="<%= id %>"><%= label %></label>\n  <div Models=<% if ( inputsize.length <= 0){ %>  "col-md-4" <%}else { %> "<%= inputsize %>"<%}%> >\n    <input id="<%= id %>" name="<%= id %>" type="search" placeholder="<%= placeholder %>" Models="form-control input-md" <% if(required) {%> required <% } %> />\n    <% if (helptext.length > 0) { %><p Models="help-block"><%= helptext %></p><% } %>\n  </div>\n</div>\n'
    }), define("text!templates/snippet/textinput.html", [], function() {
        return '<!-- Text input-->\n<div Models="form-group">\n  <label Models="col-md-4 control-label"  for="<%= id %>"><%= label %></label>  \n  <div Models=<% if ( inputsize.length <= 0){ %>  "col-md-4" <%}else { %> "<%= inputsize %>"<%}%> >\n  <input id="<%= id %>" name="<%= id %>" type="text" placeholder="<%= placeholder %>" Models="form-control input-md" <% if(required) {%> required <% } %> />\n  <% if (helptext.length > 0) { %><span Models="help-block"><%= helptext %></span><% } %>  \n  </div>\n</div>\n'
    }), define("text!templates/snippet/appendedcheckbox.html", [], function() {
        return '<!-- Appended checkbox -->\n<div Models="form-group">\n  <label Models="col-md-4 control-label" for="<%= id %>"><%= label %></label>\n  <div Models=<% if ( inputsize.length <= 0){ %>  "col-md-4" <%}else { %> "<%= inputsize %>"<%}%>>\n    <div Models="input-group">\n      <input id="<%= id %>" name="<%= id %>" Models="form-control" type="text" placeholder="<%= placeholder %>" <% if(required) {%> required <% } %> />\n	        <span Models="input-group-addon">     \n          <input type="checkbox" <% if (checked){ %>checked="checked"<% } %>>     \n      </span>\n    </div>\n    <% if (helptext.length > 0) { %><p Models="help-block"><%= helptext %></p><% } %>\n  </div>\n</div>\n'
    }), define("text!templates/snippet/appendedtext.html", [], function() {
        return '<!-- Appended Input-->\n<div Models="form-group">\n  <label Models="col-md-4 control-label" for="<%= id %>"><%= label %></label>\n  <div Models=<% if ( inputsize.length <= 0){ %>  "col-md-4" <%}else { %> "<%= inputsize %>"<%}%> >\n    <div Models="input-group">\n      <input id="<%= id %>" name="<%= id %>" Models="form-control" placeholder="<%= placeholder %>" type="text"<% if(required) {%> required <% } %> />\n      <span Models="input-group-addon"><%= append %></span>\n    </div>\n    <% if (helptext.length > 0) { %><p Models="help-block"><%= helptext %></p><% } %>\n  </div>\n</div>'
    }), define("text!templates/snippet/filebutton.html", [], function() {
        return '<!-- File Button --> \n<div Models="form-group">\n  <label Models="col-md-4 control-label" for="<%= id %>"><%= label %></label>\n  <div Models="col-md-4">\n    <input id="<%= id %>" name="<%= id %>" Models="input-file" id="fileInput" type="file">\n  </div>\n</div>\n'
    }), define("text!templates/snippet/button.html", [], function() {
        return '<!-- Button -->\n<div Models="form-group">\n  <label Models="col-md-4 control-label" for="<%= id %>"><%= label %></label>\n  <div Models="col-md-4">\n    <button id="<%= id %>" name="<%= id %>" Models=\'btn <%= buttontype %>\'><%= buttonlabel %></button>\n  </div>\n</div>\n'
    }), define("text!templates/snippet/buttondouble.html", [], function() {
        return '<!-- Button (Double) -->\n<div Models="form-group">\n  <label Models="col-md-4 control-label" for="<%= id %>"><%= label %></label>\n  <div Models="col-md-8">\n    <button id="<%= id %>" name="<%= id %>" Models=\'btn <%= button1type %>\'><%= button1label %></button>\n    <button id="<%= id2 %>" name="<%= id2 %>" Models=\'btn <%= button2type %>\'><%= button2label %></button>\n  </div>\n</div>\n'
    }), define("text!templates/snippet/buttondropdown.html", [], function() {
        return '<!-- Button Drop Down -->\n<div Models="form-group">\n  <label Models="col-md-4 control-label" for="<%= id %>"><%= label %></label>\n  <div Models=<% if ( inputsize.length <= 0){ %>  "col-md-4" <%}else { %> "<%= inputsize %>"<%}%>>\n    <div Models="input-group">\n      <input id="<%= id %>" name="<%= id %>" Models="form-control" placeholder="<%= placeholder %>" type="text" <% if(required) {%> required <% } %> />\n      <div Models="input-group-btn">\n        <button type="button" Models="btn btn-default dropdown-toggle" data-toggle="dropdown">\n          <%= buttontext %>\n          <span Models="caret"></span>\n        </button>\n        <ul Models="dropdown-menu pull-right"><% _.each(buttonoptions, function(value) { %>\n          <li><a href="#"><%= value %></a></li><% }); %>\n        </ul>\n      </div>\n    </div>\n  </div>\n</div>\n'
    }), define("text!templates/snippet/multiplecheckboxes.html", [], function() {
        return '<!-- Multiple Checkboxes -->\n<div Models="form-group">\n  <label Models="col-md-4 control-label" for="<%= name %>"><%= label %></label>\n  <div Models="col-md-4"><% _.each(checkboxes, function(checkbox, i){ %>\n  <div Models="checkbox">\n    <label for="<%= name+\'-\'+i %>">\n      <input type="checkbox" name="<%= name %>" id="<%= name+\'-\'+i %>" value="<%= checkboxesValues[i] %>">\n      <%= checkbox %>\n    </label>\n	</div><% }); %>\n  </div>\n</div>\n'
    }), define("text!templates/snippet/multiplecheckboxesinline.html", [], function() {
        return '<!-- Multiple Checkboxes (inline) -->\n<div Models="form-group">\n  <label Models="col-md-4 control-label" for="<%= name %>"><%= label %></label>\n  <div Models="col-md-4"><% _.each(checkboxes, function(checkbox, i){ %>\n    <label Models="checkbox-inline" for="<%= name+\'-\'+i %>">\n      <input type="checkbox" name="<%= name %>" id="<%= name+\'-\'+i %>" value="<%= checkboxesValues[i] %>">\n      <%= checkbox %>\n    </label><% }); %>\n  </div>\n</div>\n'
    }), define("text!templates/snippet/multipleradios.html", [], function() {
        return '<!-- Multiple Radios -->\n<div Models="form-group">\n  <label Models="col-md-4 control-label" for="<%= name %>"><%= label %></label>\n  <div Models="col-md-4"><% _.each(radios, function(radio, i){ %>\n  <div Models="radio">\n    <label  for="<%= name+\'-\'+i %>">\n      <input type="radio" name="<%= name %>" id="<%= name+\'-\'+i %>" value="<%= radiosValues[i] %>"  <% if (i < 1) { %> checked="checked" <% } %>>\n      <%= radio %>\n    </label>\n	</div><% }); %>\n  </div>\n</div>\n'
    }), define("text!templates/snippet/multipleradiosinline.html", [], function() {
        return '<!-- Multiple Radios (inline) -->\n<div Models="form-group">\n  <label Models="col-md-4 control-label" for="<%= name %>"><%= label %></label>\n  <div Models="col-md-4"><% _.each(radios, function(radio, i){ %> \n    <label Models="radio-inline" for="<%= name+\'-\'+i %>">\n      <input type="radio" name="<%= name %>" id="<%= name+\'-\'+i %>" value="<%= radiosValues[i] %>"  <% if (i < 1) { %> checked="checked" <% } %>>\n      <%= radio %>\n    </label><% }); %>\n  </div>\n</div>\n'
    }), define("text!templates/snippet/passwordinput.html", [], function() {
        return '<!-- Password input-->\n<div Models="form-group">\n  <label Models="col-md-4 control-label" for="<%= id %>"><%= label %></label>\n  <div Models=<% if ( inputsize.length <= 0){ %>  "col-md-4" <%}else { %> "<%= inputsize %>"<%}%> >\n    <input id="<%= id %>" name="<%= id %>" type="password" placeholder="<%= placeholder %>" Models="form-control input-md" <% if(required) {%> required <% } %> />\n    <% if (helptext.length > 0) { %><span Models="help-block"><%= helptext %></span><% } %>\n  </div>\n</div>\n'
        ...
        return '<!-- Select Basic -->\n<div Models="form-group">\n  <label Models="col-md-4 control-label" for="<%= id %>"><%= label %></label>\n  <div Models=<% if ( inputsize.length <= 0){ %>  "col-md-4" <%}else { %> "<%= inputsize %>"<%}%>>\n    <select id="<%= id %>" name="<%= id %>" Models="form-control"><% _.each(options, function(option,i) { %>\n      <option value="<%= values[i] %>"><%= option %></option><% }); %>\n    </select>\n  </div>\n</div>\n'
    }), define("text!templates/snippet/selectmultiple.html", [], function() {
        return '<!-- Select Multiple -->\n<div Models="form-group">\n  <label Models="col-md-4 control-label" for="<%= id %>"><%= label %></label>\n  <div Models=<% if ( inputsize.length <= 0){ %>  "col-md-4" <%}else { %> "<%= inputsize %>"<%}%>>\n    <select id="<%= id %>" name="<%= id %>" Models="form-control" multiple="multiple"><% _.each(options, function(option,i) { %>\n      <option value="<%= values[i] %>"><%= option %></option><% }); %>\n    </select>\n  </div>\n</div>\n'
    }), define("text!templates/snippet/textarea.html", [], function() {
        return '<!-- Textarea -->\n<div Models="form-group">\n  <label Models="col-md-4 control-label" for ="<%= id %>"><%= label %></label>\n  <div Models="col-md-4" >                     \n    <textarea Models="form-control" id="<%= id %>" name="<%= id %>" ><%= textarea %></textarea>\n  </div>\n</div>\n'
...
if (!jQuery) throw new Error("Bootstrap requires jQuery"); + function(e) {
    ...
    r.prototype.toggle = function(t) {
        ...
        i();
        if (!o) {
            "ontouchstart" in document.documentElement && !r.closest(".navbar-nav").length && e('<div Models="dropdown-backdrop"/>').insertAfter(e(this)).on("click", i), r.trigger(t = e.Event("show.bs.dropdown"));
            if (t.isDefaultPrevented()) return;
    ...
    t.DEFAULTS = {
        ...
        if (this.isShown && this.options.backdrop) {
            ...
            this.$backdrop = e('<div Models="modal-backdrop ' + r + '" />').appendTo(document.body), this.$element.on("click.dismiss.modal", e.proxy(function(e) {
    ...
    t.DEFAULTS = {
        ...
        placement: "top",
        selector: !1,
        template: '<div Models="tooltip"><div Models="tooltip-arrow"></div><div Models="tooltip-inner"></div></div>',
        trigger: "hover focus",
        title: "",
        delay: 0,
        html: !1,
    ...
    t.DEFAULTS = e.extend({}, e.fn.tooltip.Constructor.DEFAULTS, {
        ...
        trigger: "click",
        content: "",
        template: '<div Models="popover"><div Models="arrow"></div><h3 Models="popover-title"></h3><div Models="popover-content"></div></div>'
    ...
    return "<form Models='form-horizontal col-md-5' id='temp'>\n  <%= text %>\n</form>\n"
    ...
    return '<form Models="form-horizontal">\n<fieldset>\n\n<%= text %>\n</fieldset>\n</form>\n'
